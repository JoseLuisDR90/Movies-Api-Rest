...
//Valido si esta logueado y permito crear un cine
...
//Valido si esta logueado y permito asociar una pelicula a un cine
cinemaRouter.put("/add-movie", [isAuthPassport], async (req, res, next) => {
  try {
    const { cinemaId, movieId } = req.body;
    if (!cinemaId) {
      return next(
        createError("Se necesita un id de cine válido para añadirlo", 500)
      );
    }
    if (!movieId) {
      return next(
        createError("Se necesita un id de película válido para añadirlo", 500)
      );
    }
...
//Valido si esta logueado y permito borrar un cine
